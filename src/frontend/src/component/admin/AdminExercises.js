...
function AdminExercises({ user }) {
    ...
    const [imagePreview, setImagePreview] = useState(null); // Xem trước ảnh
    ...
    const handleExSubmit = async (e) => {
        ...
        try {
            ...
            formData.append('category_id', exForm.category_id);
            formData.append('title', exForm.title);
            formData.append('description', exForm.description);
            formData.append('duration_minutes', exForm.duration_minutes);
            formData.append('difficulty_level', exForm.difficulty_level);
            formData.append('media_type', exForm.media_type);
            formData.append('media_url', exForm.media_url);
            formData.append('instructions', exForm.instructions);
    ...
    const handleExEdit = (ex) => {
        ...
        if (ex.image_url) {
            setImagePreview(`http://localhost:5000${ex.image_url}`);
        } else {
            setImagePreview(null);
        }
        setSelectedImage(null);
                ...
                <Card className="mb-4" style={{ borderColor: '#16306e' }}>
                    ...
                    <Card.Body style={{ background: '#f8f9fa' }}>
                        <Form onSubmit={handleExSubmit}>
                            ...
                            <Form.Group className="mb-3">
                                <Form.Label>Ảnh minh họa bài tập</Form.Label>
                                <Form.Control
                                    type="file"
                                    accept="image/*"
