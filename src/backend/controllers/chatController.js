...
exports.sendMessageToBot = async (req, res) => {
    try {
        ...
        const aiResult = await aiService.analyzeEmotion(message);
        if (aiResult.success) {
            detectedEmotion = aiResult.emotion;
            console.log(`  AI Phân tích: ${detectedEmotion} (${(aiResult.score * 100).toFixed(1)}%)`);
        } else {
            console.log(`  Lỗi AI (vẫn chạy tiếp): ${aiResult.error}`);
        }
