...
const Appointment = {
    create: (userId, expertId, scheduledAt, meetingType, meetingLink, note) => {
        const sql = `INSERT INTO consultation_appointments 
                     (expert_id, user_id, scheduled_at, meeting_type, status, meeting_link, created_at)
                     VALUES (?, ?, ?, ?, 'Pending', ?, NOW())`;
        return new Promise((resolve, reject) => {
            db.query(sql, [expertId, userId, scheduledAt, meetingType || 'Online', meetingLink || null], (err, result) => {
