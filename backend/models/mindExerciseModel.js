...
const MindExercise = {
    ...
    create: (exercise) => {
        ...
        const sql = `
            INSERT INTO mind_exercises (
                ...
                difficulty_level, media_type, media_url, instructions, is_active
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `;
        return new Promise((resolve, reject) => {
            db.query(
                sql,
                [category_id, title, description, duration_minutes, difficulty_level, media_type, media_url, instructions, is_active],
                (err, result) => {
                    if (err) return reject(err);
    ...
    update: (id, exercise) => {
        ...
        return new Promise((resolve, reject) => {
            db.query(
                sql,
                [category_id, title, description, duration_minutes, difficulty_level, media_type, media_url, instructions, is_active, id],
                (err, result) => {
                    if (err) return reject(err);
                    resolve(result.affectedRows > 0);
                }
